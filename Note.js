...
//ladataan työntekijöiden ilmoitukset
...
function Aclear() {
	var element = document.getElementById("adWork");
	element.innerHTML = "";
...
function getTekija(str) {
	if (str == "") {
		...
		xmlhttp.open("GET", "PHP/tekijasearch.php?q=" + str, true);
		xmlhttp.send();
	}

}

function filtertekija(arr) {
	Tclear();
	var toAdd = document.getElementById("ad");
...
window.onload = setTimeout(getAdsJob(),300), setTimeout(getAds(),300), checksession();
